...
class QwenPawAgent(ToolGuardMixin, ReActAgent):
    ...
    **must** call ``super()._acting(...)`` / ``super()._reasoning(...)``
    ...
    def __init__(
        ...
        # Initialize self-evolution tools (memory + skill_evolution)
        self._setup_self_evolution(toolkit, namesake_strategy)
    ...
    def _setup_self_evolution(
        self,
        toolkit: Toolkit,
        namesake_strategy: NamesakeStrategy = "skip",
    ) -> None:
        """Initialize and register self-evolution tools.

        Self-evolution tools enable the agent to persist knowledge across
        sessions and create/update its own skills. Inspired by Hermes-agent's
        memory and skill_manage tools.

        Registers:
        - memory: Persistent memory store (MEMORY.md + USER.md)
        - skill_evolution: Skill creation/editing/patching

        Also creates a MemoryStore instance for system prompt injection.

        Args:
            toolkit: Toolkit to register self-evolution tools to.
            namesake_strategy: Strategy to handle namesake tool functions.
        """
        workspace_dir = self._workspace_dir or WORKING_DIR
        workspace_path = Path(workspace_dir)

        # Create MemoryStore for both tool usage and prompt injection
        self._memory_store = MemoryStore(workspace_path)

        # Register memory tool
        try:
            memory_tool_func = create_memory_tool(workspace_path)
            toolkit.register_tool_function(
                memory_tool_func,
                namesake_strategy=namesake_strategy,
            )
            logger.debug("Registered self-evolution tool: memory")
        except Exception as e:
            logger.warning("Failed to register memory tool: %s", e)

        # Register skill_evolution tool
        try:
            skill_evo_func = create_skill_evolution_tool(workspace_path)
            toolkit.register_tool_function(
                skill_evo_func,
                namesake_strategy=namesake_strategy,
            )
            logger.debug("Registered self-evolution tool: skill_evolution")
        except Exception as e:
            logger.warning("Failed to register skill_evolution tool: %s", e)
    ...
    def _build_sys_prompt(self) -> str:
        ...
        # Inject self-evolution memory snapshot
        if hasattr(self, "_memory_store"):
            memory_snapshot = self._memory_store.get_snapshot()
            if memory_snapshot:
                sys_prompt = sys_prompt + "\n\n" + memory_snapshot
